...
| Plan/chemistry algorithms | Partial, improved venue-fill parity | Venue types are filled only from the selected type; the fill stage carries unmet whole-point preferences across stops, applies POL sense, and scores novelty per member. Python/TypeScript regression tests cover these rules. Full shape-stage fairness/compromise, availability/time bands, feedback learning, and generated-fixture parity are still incomplete. |
...
| Rooms and join requests | Functional API/demo request lifecycle; room discovery and host review are navigable | Requester IDs flow through pending/accepted/declined states, duplicate pending requests are idempotent, accepted membership uses the requester ID, and hosted plans stay frozen. The v4 frozen-plan shape-fit gate is enforced in API and demo modes and surfaced in discovery; auth/authorization, durable persistence, and real-time updates are still missing. Capsules remain local demo state. |
