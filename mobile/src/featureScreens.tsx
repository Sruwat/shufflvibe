...
import { getService, JoinRequestRecord, PrivacySettings, RoomRecord } from './services';
...
function JoinRoomScreen() {
  ...
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState('');
  useEffect(() => { let active = true; service.listHostedRooms().then(async items => { if (!active) return; setRooms(items); const existing = await Promise.all(items.map(async room => [room.id, (await service.getJoinRequests(room.id, 'all')).find(item => item.member_id === 'demo-visitor')] as const)); if (active) setRequests(Object.fromEntries(existing.filter((entry): entry is readonly [string, JoinRequestRecord] => Boolean(entry[1])).map(([roomId, item]) => [roomId, item]))); }).catch(cause => { if (active) setError(cause instanceof Error ? cause.message : 'Could not load hosted rooms.'); }); return () => { active = false; }; }, [service]);
  const requestJoin = async (room: RoomRecord) => { setBusy(true); setError(''); try { const result = await service.requestToJoin(room.id, 'demo-visitor', 'I would like to join this room.'); setRequests(current => ({ ...current, [room.id]: result })); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not request to join.'); } finally { setBusy(false); } };
  return <SafeAreaView style={styles.safe}><FeatureHeader label="FIND A ROOM" onBack={() => set({ screen: 'rooms' })}/><ScrollView contentContainerStyle={styles.content}><Text style={styles.eyebrow}>HOSTED NIGHTS</Text><Text style={styles.title}>Find your people</Text><Text style={styles.body}>Only hosted rooms appear here. The spec’s full frozen-plan fit gate is not implemented yet, so this demo does not claim that these rooms are a chemistry match.</Text>{rooms.map(room => { const existing = requests[room.id]; return <View key={room.id} style={styles.roomCard}><Text style={styles.routeText}>{room.title}</Text><Text style={styles.secondaryText}>{room.members?.length ?? 0} members · {room.status}</Text>{existing ? <Text style={styles.notice}>{existing.status === 'pending' ? 'Request sent · waiting for host' : 'Request ' + existing.status}</Text> : <FeatureButton label={busy ? 'Sending…' : 'Request to join'} onPress={() => requestJoin(room)}/>}</View>; })}{!rooms.length && !error && <View style={styles.route}><Text style={styles.body}>No hosted rooms are available right now.</Text></View>}{error ? <Text accessibilityRole="alert" style={styles.notice}>{error}</Text> : null}<FeatureButton label="Host a room instead" onPress={() => set({ screen: 'feature', featureMode: 'hostRoom' })} secondary/></ScrollView></SafeAreaView>;
}
